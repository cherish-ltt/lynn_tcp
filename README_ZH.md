...
| 2 (流水线)    | 64   | 128 | **103,459** | — | — | — | — |
| 2 (流水线)    | 256  | 128 | **131,482** | — | — | — | — |
| 2 (流水线)    | 1024 | 128 | **113,253** | — | — | — | — |
| 2 (流水线)    | 4096 | 128 | **69,145** | — | — | — | — |

> 📊 以上为回环单机数据：压测客户端与服务器共享 CPU，请将其作为版本间的相对对比，而非绝对容量。Model-1 的 RTT 随并发增长（典型的排队效应）；本机 Model-2 吞吐在约 1024 客户端处趋于饱和。
...
- 吞吐处于同一量级，无可辨认的回退；4096 并发下两个模型均优于 v1.x 两代（Model-2 为 v1.1.x 的 5.1 倍、v1.2.x 的 1.3 倍）。
- v2.0 最明显的改善是**稳定性**：v1.x 随并发剧烈塌陷（Model-2 较峰值跌 83~89%），v2.0 仅衰减约 12%（Model-1）/ 47%（Model-2——且使用固定的 8 深在途窗口，而非 v1.x 的无界突发）。
